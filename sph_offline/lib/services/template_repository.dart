...
class TemplateRepository {
  ...
  Future<SphTemplate?> getByName(String name) async {
    final db = await _dbHelper.database;
    final maps = await db.query('sph_templates', where: 'name = ?', whereArgs: [name], limit: 1);
    if (maps.isEmpty) return null;
    return SphTemplate.fromMap(maps.first);
  }
  ...
  Future<void> deleteByIds(List<int> ids) async {
    ...
    await db.delete('sph_templates', where: 'id IN ($placeholders)', whereArgs: ids);
  }
