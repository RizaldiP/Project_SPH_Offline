...
class SphFormNotifier extends StateNotifier<SphFormState> {
  ...
  void calculateTotals() {
    ...
    for (int i = 0; i < items.length; i++) {
      if (items[i].type == 'item') {
        ...
        final totalPrice = items[i].qty * unitPrice;
        items[i] = items[i].copyWith(unitPrice: unitPrice, totalPrice: totalPrice);
        totalMaterial += items[i].materialPrice * items[i].qty;
        totalJasa += items[i].jasaPrice * items[i].qty;
      }
    }
    final subtotal = totalMaterial + totalJasa;
    final discountAmount = subtotal * (state.discount / 100);
    final afterDiscount = subtotal - discountAmount;
    final ppnAmount = afterDiscount * (state.ppn / 100);
  ...
  Future<int?> save() async {
    ...
    // Auto-save to Master Template (sections with jasa items)
